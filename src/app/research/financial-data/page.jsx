...
import { LineChart, ShieldCheck, Server, Eye, DownloadCloud } from "lucide-react";
    ...
    <div className="fixed inset-0 flex items-center justify-center bg-black bg-opacity-50 backdrop-blur-sm">
      ...
      >
        ...
        >
          ...
          <option value="Market Data Aggregation">Market Data Aggregation</option>
          <option value="Regulatory Compliance & AML Research">Regulatory Compliance & AML Research</option>
          <option value="Scalable Data Processing">Scalable Data Processing</option>
        </select>
...
const Page = () => {
  const [documents, setDocuments] = useState({
    ...
    "Regulatory Compliance & AML Research": [],
    "Scalable Data Processing": []
  });

  const categoryIcons = {
    "Market Data Aggregation": <LineChart className="h-8 w-8 text-blue-600" />,
    "Regulatory Compliance & AML Research": <ShieldCheck className="h-8 w-8 text-green-500" />,
    "Scalable Data Processing": <Server className="h-8 w-8 text-purple-500" />
  }
  ...
  const fetchDocuments = async () => {
    try {
      const res = await fetch("/api/research/financial-data");
    ...
    if (res.ok) {
      ...
      try {
        await fetch("/api/audit-log", {
          ...
          body: JSON.stringify({
            action: "Upload",
            details: "In Financial Data [Research]"
          }),
        });
        setLoading(false);
      } catch (error) {
                        ...
                        onClick={async (e) => {
                          try {
                            ...
                            await fetch("/api/audit-log", {
                              ...
                              body: JSON.stringify({
                                action: "Download",
                                details: "From Financial Data [Research]",
                              }),
                            });
                          } catch (error) {
                            console.error("Failed to log audit:", error);
