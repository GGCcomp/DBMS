...
import {  ShieldCheck, FileText, Eye, DownloadCloud } from "lucide-react";
    ...
    <div className="fixed inset-0 flex items-center justify-center bg-black bg-opacity-50 backdrop-blur-sm">
      ...
      >
        ...
        >
          ...
          <option value="Automated Compliance Tracking">Automated Compliance Tracking</option>
          <option value="Regulatory Sandboxes">Regulatory Sandboxes</option>
          <option value="Smart Legal Contracts">Smart Legal Contracts</option>
        </select>
...
const Page = () => {
  ...
  const categoryIcons = {
    "Automated Compliance Tracking": <ShieldCheck className="h-8 w-8 text-green-600" />,
    "Regulatory Sandboxes": <FileText className="h-8 w-8 text-yellow-500" />,
    "Smart Legal Contracts": <ShieldCheck className="h-8 w-8 text-red-500" />
  }
  ...
  const fetchDocuments = async () => {
    try {
      const res = await fetch("/api/research/regulatory-compliance");
    ...
    if (res.ok) {
      ...
      try {
        await fetch("/api/audit-log", {
          ...
          body: JSON.stringify({
            action: "Upload",
            details: "In Regulatory Compliance [Research]"
          }),
        });
        setLoading(false);
      } catch (error) {
  ...
  return (
    <div className="min-h-screen bg-gradient-to-r from-blue-500 to-purple-500 p-6 flex flex-col items-center">
      ...
      >
        Regulatory Compliance & Ethical AI Research
      </motion.h1>
                        ...
                        onClick={async (e) => {
                          try {
                            ...
                            await fetch("/api/audit-log", {
                              ...
                              body: JSON.stringify({
                                action: "Download",
                                details: "From Regulatory Compliance [Research]",
                              }),
                            });
                          } catch (error) {
                            console.error("Failed to log audit:", error);
