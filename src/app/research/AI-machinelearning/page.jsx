...
import { BrainCircuit, Search, ShieldCheck, Eye, DownloadCloud } from "lucide-react";
    ...
    <div className="fixed inset-0 flex items-center justify-center bg-black bg-opacity-50 backdrop-blur-sm">
      ...
      >
        ...
        >
          ...
          <option value="Predictive Analytics & Risk Modeling">Predictive Analytics & Risk Modeling</option>
          <option value="Natural Language Processing (NLP) for Finance">Natural Language Processing (NLP) for Finance</option>
...
const Page = () => {
  ...
  const categoryIcons = {
    "Predictive Analytics & Risk Modeling": <Search className="h-8 w-8 text-green-500" />,
    "Natural Language Processing (NLP) for Finance": <BrainCircuit className="h-8 w-8 text-purple-500" />,
    "AI Fairness & Compliance": <ShieldCheck className="h-8 w-8 text-yellow-500" />
  }
  ...
  const fetchDocuments = async () => {
    try {
      const res = await fetch("/api/research/ai-learning");
    ...
    if (res.ok) {
      ...
      try {
        await fetch("/api/audit-log", {
          ...
          body: JSON.stringify({
            action: "Upload",
            details: "In AI Learnings [Research]"
          }),
        });
        setLoading(false);
      } catch (error) {
                        ...
                        onClick={async (e) => {
                          try {
                            ...
                            await fetch("/api/audit-log", {
                              ...
                              body: JSON.stringify({
                                action: "Download",
                                details: "From AI Learnings [Research]",
                              }),
                            });
                          } catch (error) {
                            console.error("Failed to log audit:", error);
