...
import {  Cloud, FileText, Eye, DownloadCloud } from "lucide-react";
    ...
    <div className="fixed inset-0 flex items-center justify-center bg-black bg-opacity-50 backdrop-blur-sm">
      ...
      >
        ...
        >
          ...
          <option value="Jupyter Notebooks & Data Science Tools">Jupyter Notebooks & Data Science Tools</option>
          <option value="Cloud Compute Resources">Cloud Compute Resources</option>
          <option value="Secure Multi-Party Computation">Secure Multi-Party Computation</option>
        </select>
...
const Page = () => {
  ...
  const categoryIcons = {
    "Jupyter Notebooks & Data Science Tools": <Cloud className="h-8 w-8 text-green-600" />,
    "Cloud Compute Resources": <FileText className="h-8 w-8 text-blue-500" />,
    "Secure Multi-Party Computation": <Cloud className="h-8 w-8 text-yellow-500" />
  }
  ...
  const fetchDocuments = async () => {
    try {
      const res = await fetch("/api/research/secure-collaboration");
    ...
    if (res.ok) {
      ...
      try {
        await fetch("/api/audit-log", {
          ...
          body: JSON.stringify({
            action: "Upload",
            details: "In Cloud-Based Research [Research]"
          }),
        });
        setLoading(false);
      } catch (error) {
  ...
  return (
    <div className="min-h-screen bg-gradient-to-r from-blue-500 to-purple-500 p-6 flex flex-col items-center">
      ...
      >
        Cloud-Based Research & Secure Collaboration
      </motion.h1>
                        ...
                        onClick={async (e) => {
                          try {
                            ...
                            await fetch("/api/audit-log", {
                              ...
                              body: JSON.stringify({
                                action: "Download",
                                details: "From Cloud-Based Research [Research]",
                              }),
                            });
                          } catch (error) {
                            console.error("Failed to log audit:", error);
