...
import { TrendingUp, BarChart, Eye, DownloadCloud } from "lucide-react";
    ...
    <div className="fixed inset-0 flex items-center justify-center bg-black bg-opacity-50 backdrop-blur-sm">
      ...
      >
        ...
        >
          ...
          <option value="Monte Carlo & Stress Testing Models">Monte Carlo & Stress Testing Models</option>
          <option value="Derivative Pricing & Liquidity Risk">Derivative Pricing & Liquidity Risk</option>
        </select>
...
const Page = () => {
  const [documents, setDocuments] = useState({
    "Monte Carlo & Stress Testing Models": [],
    "Derivative Pricing & Liquidity Risk": []
  });

  const categoryIcons = {
    "Monte Carlo & Stress Testing Models": <TrendingUp className="h-8 w-8 text-blue-600" />,
    "Regulatory Compliance & AML Research": <BarChart className="h-8 w-8 text-green-500" />
  }
  ...
  const fetchDocuments = async () => {
    try {
      const res = await fetch("/api/research/simulation");
    ...
    if (res.ok) {
      ...
      try {
        await fetch("/api/audit-log", {
          ...
          body: JSON.stringify({
            action: "Upload",
            details: "In Simulation [Research]"
          }),
        });
        setLoading(false);
      } catch (error) {
                        ...
                        onClick={async (e) => {
                          try {
                            ...
                            await fetch("/api/audit-log", {
                              ...
                              body: JSON.stringify({
                                action: "Download",
                                details: "From Simulation [Research]",
                              }),
                            });
                          } catch (error) {
                            console.error("Failed to log audit:", error);
