...
import { BookOpen, Video, Users, Lightbulb, Eye, DownloadCloud } from "lucide-react";
    ...
    <div className="fixed inset-0 flex items-center justify-center bg-black bg-opacity-50 backdrop-blur-sm">
      ...
      >
        ...
        >
          ...
          <option value="Internal Research Wiki">Internal Research Wiki</option>
          <option value="Live Research Seminars & Webinars">Live Research Seminars & Webinars</option>
          <option value="Cross-Team Collaboration">Cross-Team Collaboration</option>
          <option value="Innovation Challenges & Hackathons">Innovation Challenges & Hackathons</option>
        </select>
...
const Page = () => {
  const [documents, setDocuments] = useState({
    ...
    "Live Research Seminars & Webinars": [],
    "Cross-Team Collaboration": [],
    "Innovation Challenges & Hackathons": []
  });

  const categoryIcons = {
    "Internal Research Wiki": <BookOpen className="h-8 w-8 text-yellow-500" />,
    "Live Research Seminars & Webinars": <Video className="h-8 w-8 text-blue-600" />,
    "Cross-Team Collaboration": <Users className="h-8 w-8 text-purple-500" />,
    "Innovation Challenges & Hackathons": <Lightbulb className="h-8 w-8 text-green-500" />
  }
  ...
  const fetchDocuments = async () => {
    try {
      const res = await fetch("/api/research/knowledgebase");
    ...
    if (res.ok) {
      ...
      try {
        await fetch("/api/audit-log", {
          ...
          body: JSON.stringify({
            action: "Upload",
            details: "In Knowledgebase [Research]"
          }),
        });
        setLoading(false);
      } catch (error) {
                        ...
                        onClick={async (e) => {
                          try {
                            ...
                            await fetch("/api/audit-log", {
                              ...
                              body: JSON.stringify({
                                action: "Download",
                                details: "From Knowledgebase [Research]",
                              }),
                            });
                          } catch (error) {
                            console.error("Failed to log audit:", error);
