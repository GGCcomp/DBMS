...
import {  Code,  Beaker, Network, Eye, DownloadCloud } from "lucide-react";
    ...
    <div className="fixed inset-0 flex items-center justify-center bg-black bg-opacity-50 backdrop-blur-sm">
      ...
      >
        ...
        >
          ...
          <option value="Low-Code & AI-Powered Prototyping">Low-Code & AI-Powered Prototyping</option>
          <option value="Proof-of-Concept (PoC) Testing">Proof-of-Concept (PoC) Testing</option>
          <option value="DeFi Experimentation & Financial Infrastructure">DeFi Experimentation & Financial Infrastructure</option>
        </select>
...
const Page = () => {
  const [documents, setDocuments] = useState({
    "Low-Code & AI-Powered Prototyping": [],
    "Proof-of-Concept (PoC) Testing": [],
    "DeFi Experimentation & Financial Infrastructure": []
  });

  const categoryIcons = {
    "Low-Code & AI-Powered Prototyping": <Code className="h-8 w-8 text-blue-600" />,
    "Proof-of-Concept (PoC) Testing": <Beaker className="h-8 w-8 text-purple-500" />,
    "DeFi Experimentation & Financial Infrastructure": <Network className="h-8 w-8 text-green-500" />
  }
  ...
  const fetchDocuments = async () => {
    try {
      const res = await fetch("/api/research/prototyping");
    ...
    if (res.ok) {
      ...
      try {
        await fetch("/api/audit-log", {
          ...
          body: JSON.stringify({
            action: "Upload",
            details: "In Prototyping & Experimentation [Research]"
          }),
        });
        setLoading(false);
      } catch (error) {
  ...
  return (
    <div className="min-h-screen bg-gradient-to-r from-blue-500 to-purple-500 p-6 flex flex-col items-center">
      ...
      >
        Prototyping & Experimentation Dashboard
      </motion.h1>
                        ...
                        onClick={async (e) => {
                          try {
                            ...
                            await fetch("/api/audit-log", {
                              ...
                              body: JSON.stringify({
                                action: "Download",
                                details: "From Prototyping & Experimentation [Research]",
                              }),
                            });
                          } catch (error) {
                            console.error("Failed to log audit:", error);
