...
const Modal = ({ title, defaultTitle = "", defaultFiles = [], defaultDepartment = "", onClose, onSave, loading }) => {
    ...
    <div className="fixed inset-0 flex items-center justify-center bg-black bg-opacity-50 backdrop-blur-sm">
      ...
      >
        ...
        <form onSubmit={async (e) => {
          e.preventDefault();
          await onSave(inputTitle, selectedDepartment, selectedFiles);
        }}>
          <input
            type="text"
            value={inputTitle}
...
const Page = () => {
  ...
  const [modalData, setModalData] = useState({ open: false, title: "", files: [], department: "" });


  const handleOpenModal = () => setModalData({ open: true, title: "", files: [], department: "" });

  const fetchDocuments = async (pageNumber = 1, searchQuery = "") => {
    try {
      setLoading(true);
      const res = await fetch(
        `/api/hr/data-submission?page=${pageNumber}&search=${encodeURIComponent(searchQuery)}`
      );
  ...
  const handleSaveDocument = async (title, department, files) => {
    setUploading(true);
    ...
    const res = await fetch("/api/hr/data-submission", {
      method: "POST",
      body: formData,
    });
    ...
    if (res.ok) {
      try {
        await fetch("/api/audit-log", {
          ...
          body: JSON.stringify({
            action: "Upload",
            details: "In HR Data-Submission"
          }),
        });
      } catch (error) {
        console.error("Failed to log audit:", error);
      }
      setModalData({ open: false, title: "", files: [], department: "" });
      setUploading(false);
      fetchDocuments(page);
    }
  };
    ...
    <div className="min-h-screen bg-gradient-to-r from-blue-500 to-purple-500 p-6 flex flex-col items-center">
            ...
            {documents.length > 0 ? (
              documents.map((doc, index) => (
                <tr key={index} className="hover:bg-gray-50">
                  ...
                  <td className="p-2 border capitalize">{doc.department}</td>
      ...
      {modalData.open && (
        <Modal
          ...
          defaultDepartment={modalData.department}
          defaultLink={modalData.link}
          onClose={() => setModalData({ open: false, title: "", files: [], department: ""})}
