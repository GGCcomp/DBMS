...
export default function Page() {
  ...
  const addTicket = () => {
    if (newIssue.trim()) {
      setTickets([...tickets, { id: Date.now(), issue: newIssue, status: "In Progress" }]);
      setNewIssue("");
    }
  };
      ...
      >
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
          <div className="bg-gray-100 p-6 rounded-lg shadow-md">
            <h2 className="text-2xl font-semibold text-gray-700 mb-4">Ticketing System & Request Tracking</h2>
            <input 
              type="text" 
            ...
            />
            <button onClick={addTicket} className="bg-pink-500 text-white px-4 py-2 rounded-lg w-full mb-4">Add Ticket</button>
            <ul className="text-gray-600 text-left space-y-2">
              {tickets.map(ticket => (
                <li key={ticket.id} className="flex justify-between items-center bg-white p-3 rounded shadow">
                  <span>📌 <strong>{ticket.issue}</strong> (<span className={ticket.status === "Resolved" ? "text-green-600" : "text-yellow-600"}>{ticket.status}</span>)</span>
                  {ticket.status !== "Resolved" && (
                    <button onClick={() => markResolved(ticket.id)} className="bg-green-500 text-white px-3 py-1 rounded">Resolve</button>
                  )}
                </li>
              ))}
            </ul>
